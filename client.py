...
class GUI:
    ...
    def chat_layout(self, chatname):
        ...
        self.chatwindow.configure(width = 500, height = 550, bg = "#17202A")
    ...
    def receive(self):
        while True:
            try:
                ...
                if incoming[:5] == ":left": # server lets this thread know that the main thread quit the chat
                    break
                elif incoming[:12] == ":chathistory":
                    for line in incoming[12:].split("\n"):
                        self.handle(line)
                else:

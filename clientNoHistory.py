...
class GUI:
    ...
    def receive(self):
        '''
        Operates as thread to receive and insert messages from server into the text console
        '''
        while True:
            try:
                message = c.recv(MSG_SIZE).decode() # receive message/update from server
                if message[:5] == ":left": 
                    # server lets this thread know that the main thread quit the chat
                    break
                elif message[:15] == ":newparticipant": 
...
def main():
    ...
    try:
        ...
        # create & place label
        down = Label(server_down, text = "Couldn't locate server address.\nPlease make sure server_address_nohistory.txt is downloaded.", font = "Helvetica 14 bold")
